...
OUTBOUND_CALLS = sorted(set(re.findall(r"(?:req|requests|SESSION|WEBHOOK_SESSION|session|temp_session)\.(?:get|post|head)\([^\n]*", SOURCE)))
...
def test_the_outbound_request_sweep_finds_the_call_sites():
    assert len(OUTBOUND_CALLS) >= 20, OUTBOUND_CALLS
