...
SOURCE = (PROJECT_ROOT / "spotify_profile_monitor.py").read_text(encoding="utf-8")
...
# A guard against the sweep silently matching nothing after a rename: the tool has 26 call sites today
MINIMUM_HTTP_CALL_SITES = 26
...
def test_every_outbound_request_passes_the_setting():
    ...
    assert not missing, f"spotify_profile_monitor.py lines {missing} make an HTTP call that does not pass the TLS setting"
...
def test_every_outbound_request_carries_a_deadline():
    ...
    assert not missing, f"spotify_profile_monitor.py lines {missing} make an HTTP call without a timeout"
