...
def summary_lines(**overrides):
    return {row.label: monitor._format_startup_summary_row(row) for row in monitor.build_startup_summary("target", None, None, None, **overrides)}
...
# Verifies the attachment setting is reported next to the channel that uses it
def test_the_email_image_row_reports_the_setting(monkeypatch):
    monkeypatch.setattr(monitor, "EMAIL_IMAGES", True)

    assert summary_values()["Email images"] == "True"
