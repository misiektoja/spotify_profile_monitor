...
def test_invalid_runtime_timing(monkeypatch, value):
    ...
    validate = monitor.runtime_numeric_errors
...
def test_invalid_collection(tmp_path, record):
    ...
    with pytest.raises(ValueError):
        monitor.read_collection_record(path)
...
def test_partial_collection_with_extra_metadata(tmp_path):
    path = tmp_path / "collection.json"
    record = [10, [{"uri": "spotify:user:someone", "name": "Someone"}], {"note": "kept by the operator"}]
    path.write_text(json.dumps(record), encoding="utf-8")
    assert monitor.read_collection_record(path) == record
