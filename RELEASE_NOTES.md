...
Version **3.9** gives **every rejected setup answer a way out**, **checks the mail server during setup**, adds **`--set-smtp-password`** command for hidden password entry, adds the **install method and effective secret origins** to diagnostics, makes an **exported secret win over the dotenv file**, makes **debug traces scannable** and makes setup polling formats explicit.

**Features and improvements**:

- **NEW:** **Private SMTP password setup** - `--set-smtp-password` takes a hidden password and checks it with the mail server before saving. Guided setup now checks email credentials too, without sending a message
...
- **IMPROVE:** **Setup preserves your progress** - Skip an answer you cannot provide without losing earlier answers. Reruns use saved settings as defaults. Polling prompts show accepted duration formats and monitoring is offered after Doctor passes
- **IMPROVE:** **More useful Doctor reports** - Reports name output destinations and enabled alert categories, warn about polling below 30 seconds and include approved delivery tests in the final verdict
- **IMPROVE:** **Quieter diagnostics** - `--verbose` focuses on operational changes. `--debug` provides clearer operation traces. Detailed startup output identifies the install method and effective secret sources without revealing values
- **IMPROVE:** **TLS verification includes email** - `VERIFY_SSL` now also controls mail-server certificate checks. Startup and Doctor warn when verification is disabled
- **IMPROVE:** **Clearer colours and help** - Names, IDs and links are easier to distinguish. Existing colour overrides still work. Remove the old `COLOR_THEME` block to follow the updated defaults

**Bug fixes**:

- **BUGFIX:** **Cleared secrets leave the dotenv file** - Declining the ntfy access token during setup removes its line instead of leaving an empty `NTFY_ACCESS_TOKEN` behind for the next run to read
...
- **BUGFIX:** **Less repetitive outage output** - Persistent failures use periodic reminders instead of repeating full errors, followed by a recovery notice. Liveness messages now follow elapsed time and appear in every output mode
- **BUGFIX:** **Reliable setup and recovery commands** - Printed commands retain the target and applicable file choices, including usable cookie-import destinations. Ctrl+C ends optional delivery tests and setup correctly reports whether settings were saved
- **BUGFIX:** **Consistent notification settings** - Declining email setup clears old settings. Unconfigured webhooks are disabled and incomplete email settings produce a Doctor warning that names what is missing
- **BUGFIX:** **Correct terminal formatting** - Names with apostrophes remain fully coloured, truncation ignores invisible colour codes and `--no-color` also disables Python 3.14 help colours
