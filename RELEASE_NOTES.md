...
- **BUGFIX:** **Spotify requests stay on Spotify** - Paginated follower, playlist and track requests now verify that each server-supplied page URL is an HTTPS Spotify host before sending your access token to it, and stop after 1000 pages. A tampered API response can no longer redirect a credential-bearing request to another host or drive an endless fetch loop. Identifiers taken from API responses are also percent-encoded, so a malformed collaborator ID cannot reach a different Spotify endpoint
