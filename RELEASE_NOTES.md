...
- **BUGFIX:** **Network failures point at the right page** - A timed-out or unreachable Spotify request ended with a **`Guide:`** link to the **Debugging Tools** page, which covers token utilities and says nothing about connection problems, and told you to run `--doctor --debug` for a check the monitor retries on its own. A Spotify server error sent you to the same page. Those reports now link to the new **Connection Problems** section, which explains the automatic retries and what to check if the failure continues. The file descriptor limit has its own section and an unreadable or unwritable file links to the configuration page
...
- **BUGFIX:** **Follower history saved by 3.8 loads again** - Releases before 3.9 wrote `null` into the follower, following or playlist history file when Spotify did not return that data, which is the norm for followers and followings in cookie mode. The stricter file check in 3.9 refused such a file at startup with **`saved collection entries must be a list of objects`** and stopped the run. Those files now load, a missing list is treated as empty and a missing count starts a new baseline, so upgrading no longer requires deleting the history files
