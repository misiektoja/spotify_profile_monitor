...
- **CONFIG CHANGE:** **Configuration files are read as data** - The config file is parsed instead of executed, so a `spotify_profile_monitor.conf` sitting in your working directory can no longer run code when the tool starts. Files may contain only `NAME = value` assignments using documented settings and plain literals (numbers, quoted strings, `True`, `False`, `None`, lists, tuples and dicts). Generated configs are unaffected. If you hand-wrote an expression such as `SPOTIFY_CHECK_INTERVAL = 30 * 60`, replace it with the computed value (`1800`) - the tool now names the exact line instead of failing obscurely
